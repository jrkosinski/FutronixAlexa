...
/* Experimental HttpClient for Edison that takes a curl command. This class is needed for
 * testing purposes */
class EdisonHttpCurlClient: public IHttpClient {
public:
    EdisonHttpCurlClient();
    /* Send http request and return the response.*/
    char* send(const char *request, const char* serverUrl, int port);
