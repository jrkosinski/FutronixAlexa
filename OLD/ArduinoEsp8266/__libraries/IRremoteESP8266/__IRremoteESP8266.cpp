 ...
 * Updated by Sebastien Warin (http://sebastien.warin.fr) for re ceiving IR code
 *   on ESP8266
 *
...
#define SEND_ONE this->mark
#define SEND_ZERO this->space

#define REV_SEND_ONE this->space
#define REV_SEND_ZERO this->mark


#define FUTRONIX_BIT_MARK    		450
#define FUTRONIX_INTERVAL    		1560
#define FUTRONIX_HDR_MARK    		450
#define FUTRONIX_HDR_SPACE   		((FUTRONIX_INTERVAL*4)-FUTRONIX_BIT_MARK)
#define FUTRONIX_ONE_SPACE_ODD   	((FUTRONIX_INTERVAL*2)-FUTRONIX_BIT_MARK)
#define FUTRONIX_ONE_SPACE_EVEN   	((FUTRONIX_INTERVAL*3)-FUTRONIX_BIT_MARK)
#define FUTRONIX_ZERO_SPACE  		(FUTRONIX_INTERVAL-FUTRONIX_BIT_MARK)

void IRsend::sendFutronix(unsigned long command) {
  // Set IR carrier frequency
  enableIROut(38);
  
  //How long is each 'pulse' (prelim, start, and stop)? 
  //Why is the glitch filter mentioned? 
  //Is the value big-endian or little-endian? I assume big-endian 
  //How is it 21 in total duration? I only see 20. 
  //Is there a mark 4T before the final pulse? 
  //Is the parity bit, if 1, 2T or 3T? 
  ...
  */
  ...
  for(int n=0; n<1; n++)
  {
	  ...
	  {
		  footerType = i; 
		  //this->doFutronix(command, 100, headerType, footerType); 
		  //this->IRpin++; 
		  //this->doFutronix(command, 200, headerType, footerType); 
		  //this->IRpin++; 
		  //this->doFutronix(command, 300, headerType, footerType); 
		  //this->IRpin++; 
		  //this->doFutronix(command, 400, headerType, footerType); 
		  //this->IRpin++; 
		  //this->doFutronix(command, 500, headerType, footerType); 
		  //this->IRpin++; 
		  //this->doFutronix(command, 600, headerType, footerType); 
		  //this->IRpin++; 
		  //this->doFutronix(command, 700, headerType, footerType); 
		  //this->IRpin++; 
		  this->doFutronix(command, 768, headerType, footerType); 
		  //this->IRpin++; 
		  this->doFutronix(command, 800, headerType, footerType); 
		  //this->IRpin++; 
		  this->doFutronix(command, 900, headerType, footerType); 
		  //this->IRpin++; 
		  //this->doFutronix(command, 1560, headerType, footerType); 
		  //this->IRpin++; 
  ...
  //address 
  SEND_ZERO(interval * 4); 		//bit 1 
  //space(interval * 1); 		//bit 2 
  //space(interval * 1); 		//bit 3 
  //space(interval * 1); 		//bit 4 

  //command 
  SEND_ZERO(interval * 1); 		//bit 5 
  SEND_ONE(interval * 3); 		//bit 6doFut
  SEND_ONE(interval * 3); 		//bit 6
  SEND_ZERO(interval * 1); 		//bit 7 
  SEND_ZERO(interval * 1); 		//bit 8 
  SEND_ZERO(interval * 1); 		//bit 9 
  SEND_ONE(interval * 3); 		//bit 10 
  
  //parity bit 
  SEND_ONE(interval); 

  //footer
  SEND_ZERO((interval * 3)); 
  SEND_ONE(interval);
  ledOff();
  delay(1000); 
  ...
  //header
  REV_SEND_ONE(interval * 4);
  REV_SEND_ZERO(interval); 

  //address 
  REV_SEND_ZERO(interval * 4); 		//bit 1 
  //space(interval * 1); 		//bit 2 
  //space(interval * 1); 		//bit 3 
  //space(interval * 1); 		//bit 4 

  //command 
  REV_SEND_ZERO(interval * 1); 		//bit 5 
  REV_SEND_ONE(interval * 3); 		//bit 6
  REV_SEND_ZERO(interval * 1); 		//bit 7 
  REV_SEND_ZERO(interval * 1); 		//bit 8 
  REV_SEND_ZERO(interval * 1); 		//bit 9 
  REV_SEND_ONE(interval * 3); 		//bit 10 
  
  //parity bit 
  REV_SEND_ONE(interval); 

  //footer
  REV_SEND_ZERO((interval * 3)); 
  REV_SEND_ONE(interval);
  ledOff();
  delay(1000); 
  */
  ...
  //header
  mark(FUTRONIX_HDR_MARK); 
  space(FUTRONIX_HDR_SPACE); 
  mark(FUTRONIX_HDR_MARK); 
  space(FUTRONIX_ZERO_SPACE); 
  ...
  for (unsigned long mask = 1UL << (nbits - 1); mask; mask >>= 1) {
    if (command & mask) {  // 1
      mark(FUTRONIX_BIT_MARK);
	  if (bitPosition % 2 == 0)
		space(FUTRONIX_ONE_SPACE_EVEN);
	  else 
		space(FUTRONIX_ONE_SPACE_ODD);
	  onesCount++; 
    } else { // 0
      mark(FUTRONIX_BIT_MARK);
      space(FUTRONIX_ZERO_SPACE);
  ...
  //header
  //mark(bitmark); 
  //space((interval*4) - bitmark); 
  space((interval*4)); 

  mark(bitmark); 
  space(interval - bitmark); 
  ...
  //address 
  space(interval * 1); 		//bit 1 
  space(interval * 1); 		//bit 2 
  space(interval * 1); 		//bit 3 
  space(interval * 1); 		//bit 4 

  //command 
  space(interval * 1); 		//bit 5 
  mark(interval * 3); 		//bit 6
  space(interval * 1); 		//bit 7 
  space(interval * 1); 		//bit 8 
  mark(interval * 2); 		//bit 9 
  mark(interval * 3); 		//bit 10 
...
void IRsend::doFutronix(unsigned long data, int bitmark, int headerType, int footerType)
{
  ...
  switch(headerType)
  {
	  ...
	  case 2: 
		  mark(bitmark * 4); 
		  space((FUTRONIX_INTERVAL * 1)); 
		  break;
	  case 3: 
		  mark(bitmark); 
		  space((FUTRONIX_INTERVAL*4) -bitmark); 
		  mark(FUTRONIX_INTERVAL); 
		  break;
  }
